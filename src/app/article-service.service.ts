...
export class ArticleServiceService {
  ...
  allArticles$: Observable<ArticleInterface[]>;
  articleSelected: any;

  constructor(){
    const articlesCollection = collection(this.firestore, 'articles');
    this.allArticles$ = collectionData(articlesCollection, {
      idField: 'id',
  ...
  getArticles(): Observable<any>{
    return this.allArticles$.subscribe(each => console.log(each));
  }
  // Get one article
  getArticle(id:number){
    this.articleSelected = this.allArticles$.subscribe(each => console.log(each));
